 ...
 * =========================================================================
 *
 *   Copyright (c) 2019-2024, Aquen OSS (https://aquen.io)
 *
...
public class AtomicHashStoreComputeIfAbsentTest {
    ...
    private static <K,V> AtomicHashStore<K,V> add(final AtomicHashStore<K,V> store, final K key, final V value) {
        ...
        store2 = store.computeIfAbsent(key, (k) -> value, (v) -> { m.put("VALUE", v);});
        if (oldContainsKey) {
        ...
        store2 = store.computeIfAbsent(key, (k) -> value);
        ...
        Assertions.assertTrue(value == null || newContainsKey);
        if (!oldContainsKey && (!oldContainsValue || oldValue == null)) {
            Assertions.assertTrue(value == null || newContainsValue);
            Assertions.assertEquals((oldContainsKey || value == null) ? oldSize : (oldSize + 1), newSize);
